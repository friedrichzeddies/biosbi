...
- **Step 1: Sample the "True" Parameter $\theta^*$.**
  ...
  _Crucially_: Even though we only care about the rank of the index, we must still sample all "nuisance parameters" (like random rotations and noise levels) from their respective priors to simulate a realistic image that matches the model's training experience.
