...
**For a brief moment now, we will only be interested in the cat as a 2D object.** At first, this might sound confusing with the explanation given above, but it makes life easier for a few minutes. From the model, we take just a "picture" (rather a 2D projection) for now and manipulate it. That action will be justified once looking at the optional sub-chapter of the _Ewald sphere_ and _projection slice theorem_, if you are interested.
