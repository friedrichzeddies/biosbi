...
### 6. High-Integrity Posterior Visualization
When constructing inference dashboards (like `03_01_ill-posedness.py`) that must dynamically serve an arbitrary number of distinct 3D conformations (from 2 up to 100), follow these core agentic design patterns for a "Gold Standard" posterior plot:

- **De-Quantization KDE Boundaries:** Normalizing flows predicting discrete integer classes (e.g. Conformation 0, 1, 2) often require the integer boundaries to be uniformly "de-quantized" with $\pm 0.5$ noise. Use `ax.set_xlim(-0.5, num_models - 0.5)` and draw vertical dashed lines at `i - 0.5`.
- **Ideal Range Highlighting:** Highlight the target bin ($[i-0.5, i+0.5]$) using `ax.axvspan`. Due to the continuous uniform sampling during training, a perfectly calibrated model should ideally produce a **uniform distribution** filling this entire shaded box.
- **Lifted Estimator Statistics:** Instead of separate metrics, integrate the mean and standard deviation directly into the KDE plot as "strich" markers with error bars. Lift them to a cleaner y-position (e.g., `y=0.1`) for better visibility: `ax.errorbar(mean, 0.1, xerr=std, fmt='|', ...)`.
- **Integrated Labeling:** Ensure the legend is informative, grouping the Posterior, Ideal Range, and Estimator Stats per model/slot for a concise, descriptive UI.
- **Graceful Simulator Fallbacks:** If a user selects a model directory without a trained `estimator.pt`, use `os.path.exists` early. Gracefully load the `CryoEmSimulator` anyway for simulation visualization and replace the KDE block with an `st.info()` warning.

#### Interpreting the Posterior Plot
When explaining the posterior to users, emphasize that the KDE represents a smooth approximation of the neural posterior $q(\theta | x)$ estimated from many samples (typically ~2000). 
- **Training Prior:** During training, the conformation index $\theta$ is sampled from a continuous uniform distribution (e.g., $\theta = 0.4434$).
- **Discretization:** Before simulation, $\theta$ is rounded to the nearest model index (Model 0 in this case).
- **Ideal Posterior:** Because of this, the ideally learned posterior should fill the entire 'partition' (shaded box) uniformly. Broad peaks spanning multiple partitions indicate visual ambiguity in the 2D projection.
